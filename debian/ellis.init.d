...
NAME=ellis       # Introduce the short server's name here (must match the process name as set by
                 # prctl in main.py)
...
do_stop()
{
  ...
  # the parent - but we specify a process name and user, so that we don't kill other things being
  # run by /usr/share/clearwater/ellis/env/bin/python, like create_numbers.py.
  start-stop-daemon --stop --quiet --retry=TERM/30/KILL/5 --name $NAME --user $USER --exec $DAEMON
  RETVAL="$?"
  [ "$RETVAL" = 2 ] && return 2
...
do_abort()
{
  ...
  #   2 if daemon could not be stopped
  #   other if a failure occurred
  start-stop-daemon --stop --quiet --retry=USR1/5/TERM/30/KILL/5 --name $NAME --user $USER --exec $DAEMON
  RETVAL="$?"
  [ "$RETVAL" = 2 ] && return 2
  # Many daemons don't delete their pidfiles when they exit.
  rm -f $PIDFILE
